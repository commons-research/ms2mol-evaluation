...
class IcebergEvaluation(Evaluation):
    def __init__(
        self,
        output_dir: Path,
        ...
        self.df_file_path = self.output_dir / "iceberg_scores.csv"
        if os.path.exists(self.df_file_path):
            os.remove(self.df_file_path)

        self.isdb_spectra = self._load_iceberg_spectra(Path("data/iceberg_isdb.mgf"))
    ...
    def _load_iceberg_spectra(self, mgf_path: Path) -> List[Spectrum]:
        """Load Iceberg spectra from an MGF file."""
        spectra = []
        for spec in tqdm(load_from_mgf(mgf_path), desc="Loading Iceberg ISDB spectra"):
            spectra.append(default_filters(spec))
        ...
        spectra = [
            ...
            for s in tqdm(
                spectra, desc="Converting MatchMS spectra to custom spectrum objects"
            )
        ]
        return spectra
